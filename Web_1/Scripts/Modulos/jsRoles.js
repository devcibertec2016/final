$(document).ready(function () {
    ...
    $("#btnRegistrar").on("click", function () {
        if (fnValidaCampos()) {
            fn_msjConfirmacion({
                title: "CONFIRMACIÓN",
                text: "¿Estás seguro de registrar los datos del Rol ?",
    ...
    $("#btnEliminar").on("click", function () {
        fn_msjConfirmacion({
            title: "CONFIRMACIÓN",
            text: "¿Estás seguro de eliminar los datos del Rol ?",
            confirm: function (button) {
                //LoadingDialog.show();
                $("#EliminarRol").submit();
            },
    ...
    $("#btnEditar").on("click", function () {
        if (fnValidaCampos()) {
            fn_msjConfirmacion({
                title: "CONFIRMACIÓN",
                text: "¿Estás seguro de actualizar los datos del Rol ?",
                confirm: function (button) {
                    //LoadingDialog.show();
                    $("#EditarRol").submit();
                },
...
function fnInicializaVariables() {
    fnSoloLetras("vNombreRol");
    $("#vNombreRol").attr("maxlength", "30");
}
...
function fnValidaCampos() {
    ...
    var sNombreRol = $("#vNombreRol").val();
    if ($.trim(sNombreRol).length == 0) {
        br = true;
        $("#hddMessage").append("Por favor escriba el nombre del Rol <br/>");
    }
